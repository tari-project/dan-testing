...
for i in range(num_vns):
    ...
    for row in rows:
        ...
        match payload_height:
            case 1: relname = "PREPARE"
            case 2: relname = "PRECOMMIT"
            case 3: relname = "COMMIT"
            case 4: relname = "DECIDE"
        create_rel = f"MATCH (a:Node {{id: '{node_hash_hex}'}}), (b:Node {{id: '{parent_hash_hex}'}}) MERGE (a)-[hs_{node_hash_hex}_{parent_hash_hex}:{relname}]->(b)"
        print(create_rel)
        ...
        if validators_metadata:
            for vn_vote in validators_metadata:
                ...
                neo4j_graph.run(f"MATCH (a:Node {{id: '{parent_hash_hex}'}}), (b:Validator {{id: '{public_key}'}}) MERGE (b)-[vote_{parent_hash_hex}_{public_key}:VOTED]->(a) ")
                neo4j_graph.run(f"MATCH (a:Node {{id: '{node_hash_hex}'}}), (b:Validator {{id: '{public_key}'}}) MERGE (b)-[justify_{node_hash_hex}_{public_key}:JUSTIFIES]->(a)")
